class Station {
  ...
  // Computed properties
  int get availableChargers => chargers?.where((c) => c.status == 'available').length ?? 0;
  int get totalChargers => chargers?.length ?? 0;
  double get minPrice => chargers?.map((c) => c.pricePerKwh).reduce((a, b) => a < b ? a : b) ?? 0;
  double get maxPower => chargers?.map((c) => c.powerKw).reduce((a, b) => a > b ? a : b) ?? 0;
  ...
  factory Station.fromJson(Map<String, dynamic> json) {
    return Station(
      ...
      hoursJson: StationHours.fromJson(json['hours_json'] as Map<String, dynamic>),
      amenitiesJson: (json['amenities_json'] as List).cast<String>(),
      status: json['status'] as String,
      chargers: (json['chargers'] as List?)?.map((c) => Charger.fromJson(c)).toList(),
